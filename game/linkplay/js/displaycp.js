...
LinkGame.prototype.makeData = function () {
    ...
    //根据此顺序将基础数组添加到地图数组
    for (let o = 0; o < arrbase.length && o < arrorder.length; o++) {
        let x = Math.floor(arrorder[o] / this.x) + 1;
        let y = arrorder[o] % this.x + 1;
        console.log("x is " + x + " y is " + y)
        arrmap[x][y] = arrbase[o]
    }
    console.log("位置乱序：" + arrorder)
    return arrmap;
};
LinkGame.prototype.render = function () {
    for (let i = 0; i < this.y; i++) {
        for (let j = 0; j < this.x; j++) {
            ...
            let textChild = $("<p></p>");
            textChild.text(name[index]);
            child.append(textChild);
