...
LinkGame.prototype.makeData = function () {
    ...
    for (var o = arrbase.length; o < arrorder.length && o < arrbase.length + 4; o++) {
        arrmap[Math.floor(arrorder[o] / this.x) + 1][(arrorder[o] % this.x + 1)] = 11
    }
...
LinkGame.prototype.getconnect = function (a, b) {
    ...
    if (that.arrmap[ai][aj] !== 11
        && that.arrmap[bi][bj] !== 11
        && that.arrmap[ai][aj] !== that.arrmap[bi][bj]) {
